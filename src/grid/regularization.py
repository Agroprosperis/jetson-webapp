...
class LineRegularizer:
    ...
    def __init__(
        ...
        parallel_force_ratio: float = 0.45,
        spatial_reg_gain: float = 0.25,
        ...
        Args:
            ...
            parallel_force_ratio: Confidence ratio below which a line is forced
                to adopt the dominant orientation.
    ...
    def _regularize_orientation(
        ...
        max_conf = max(info["conf"] for info in regularized)
        reference_lines = [i for i in regularized if i["conf"] >= self._parallel_ref_min_ratio * max_conf]
        
        if not reference_lines:
            reference_lines = sorted(regularized, key=lambda item: item["conf"], reverse=True)[:1]

        reference_lines = sorted(reference_lines, key=lambda item: item["conf"],reverse=True)[: self._parallel_ref_max_count]
        dominant_theta = self._dominant_theta_from_clusters(reference_lines)
        self._force_parallel_lines(regularized, dominant_theta, max_conf)
    ...
    def _force_parallel_lines(
        ...
        for line in lines:
            if line["conf"] < self._parallel_force_ratio * max_conf:
                line["theta"] = dominant_theta
